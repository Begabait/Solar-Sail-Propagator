...
double R = 6.96 * pow(10, 8);
double T = 5772;
double M = 1.989 * pow(10, 30);
double st_bo = 5.67 * pow(10, -8);
double c = 3 * pow(10, 8);
double G = 6.6743 * pow(10, -11);
double S = 2.7 * pow(10, 4);
double m = 100;
double beta = 0;
...
void RHS_Function_FAKE( double* state , double* deriv_state ){
...
void RHS_Function( double* state , double* deriv_state ){
    ...
    *( deriv_state ) = *( state + 3 );
    *( deriv_state + 1 ) = *( state + 4 );
    *( deriv_state + 2 ) = *( state + 5 );
    double norm = sqrt( pow( *( state ) , 2 ) + pow( *( state + 1 ) , 2 ) + pow( *( state + 2 ) , 2 ) );    /* state + 2 changed to state + 1 cause it DOESNT MAKE SENSE!!!*/
    *( deriv_state + 3 ) = ( - *(state) / (pow(norm, 3))) * ( 1 - 1.95 * st_bo * pow(R, 2) * pow(T, 4) * S * cos(beta) / (c * m * M * G) );
    *( deriv_state + 4 ) = ( - *(state + 1) / (pow(norm, 3))) * ( 1 - 1.95 * st_bo * pow(R, 2) * pow(T, 4) * S * cos(beta) / (c * m * M * G) );
    *( deriv_state + 5 ) = ( - *(state + 2) / (pow(norm, 3))) * ( 1 - 1.95 * st_bo * pow(R, 2) * pow(T, 4) * S * cos(beta) / (c * m * M * G) );
